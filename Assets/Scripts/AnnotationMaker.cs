...
public class AnnotationMaker : MonoBehaviour
{
    ...
    public void TutorialApproveHighlightedObject()
    {
        annotate(approveMarker, rejectMarker);
    }
    public void TutorialRejectHighlightedObject()
    {
        annotate(rejectMarker, approveMarker);
    }
    ...
    private void annotate(GameObject addedMarker, GameObject removedMarker)
    {
        string addedMarkerName = addedMarker.name + "(Clone)";
        string removedMarkerName = removedMarker.name + "(Clone)";
        if (CoreServices.InputSystem.GazeProvider.GazeTarget)
